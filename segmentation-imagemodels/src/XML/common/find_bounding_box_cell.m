...
function  B = find_bounding_box_cell(T, BW_cell)

[Y,X,Z] = size(BW_cell);

% Find a bounding box
ind = find(BW_cell > 0);
[y,x,z] = ind2sub([Y,X,Z], ind);
...
B.y1 = Y - B.y1 + 1; % Map to the coordinates of the original Z-stacks
B.y2 = Y - B.y2 + 1;
B.z1 = B.z1 + T.StartingFrame;
B.z2 = B.z2 + T.StartingFrame;