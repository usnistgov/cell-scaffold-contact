...
function [status, message] = alg4_additive_gaussian_model(xmlDocument)
%% Algorithm 4: Additive Gaussian model
% Inputs
...
for kd = 1:nD
    ...
    m_bg_cell = mean2(bg_img_cell);
    s_bg_cell = std2(bg_img_cell);
    m_bg_scaffold = mean2(bg_img_scaffold);
    s_bg_scaffold = std2(bg_img_scaffold);
        
    ksize_cell = round(1.06*s_bg_cell*(H*W)^(-0.2));
    ksize_scaffold = round(1.06*s_bg_scaffold*(H*W)^(-0.2));
        
    if ksize_cell > 1
        GaussFilter_cell = fspecial('gaussian', [ksize_cell ksize_cell], 1/ksize_cell);
        if sum(GaussFilter_cell(:))==0
            GaussFilter_cell = [];
        end
    else
        GaussFilter_cell = [];
    ...
    if ksize_scaffold > 1
        GaussFilter_scaffold = fspecial('gaussian', [ksize_scaffold ksize_scaffold], 1/ksize_scaffold);
        if sum(GaussFilter_scaffold(:))==0
            GaussFilter_scaffold = [];
        end
    else
        GaussFilter_scaffold = [];
    end

    Z_Gauss_cell = zeros(L,H,W);
    Z_Gauss_scaffold = zeros(L,H,W);
    for kl = 1:L
        I_cell = squeeze(Z_cell(kl,:,:));
        I_scaffold = squeeze(Z_scaffold(kl,:,:));

        if isempty(GaussFilter_cell)
            Z_Gauss_cell(kl,:,:) = I_cell - m_bg_cell;
        else
            Z_Gauss_cell(kl,:,:) = imfilter(I_cell, GaussFilter_cell) - m_bg_cell;
        end

        if isempty(GaussFilter_scaffold)
            Z_Gauss_scaffold(kl,:,:) = I_scaffold - m_bg_scaffold;
        else
            Z_Gauss_scaffold(kl,:,:) = imfilter(I_scaffold, GaussFilter_scaffold) - m_bg_scaffold;
        end
    end

    Z_Gauss_cell(Z_Gauss_cell<0) = 0;
    Z_Gauss_scaffold(Z_Gauss_scaffold<0) = 0;

    % Probability
    max_Z_Gauss_cell = max(Z_Gauss_cell(:));
    max_Z_Gauss_scaffold = max(Z_Gauss_scaffold(:));
        
    P_cell = Z_Gauss_cell / max_Z_Gauss_cell;
    P_scaffold = Z_Gauss_scaffold / max_Z_Gauss_scaffold;
    
%     % Threshold determination (percentile)    
%     if isempty(GaussFilter_cell)
%         bg_Gauss_cell = bg_img_cell - m_bg_cell;
%     else
%         bg_Gauss_cell = imfilter(bg_img_cell, GaussFilter_cell) - m_bg_cell;
%     end
% 
%     if isempty(GaussFilter_scaffold)
%         bg_Gauss_scaffold = bg_img_scaffold - m_bg_scaffold;
%     else
%         bg_Gauss_scaffold = imfilter(bg_img_scaffold, GaussFilter_scaffold) - m_bg_scaffold;
%     end
% 
%     th_cell = compute_threshold(bg_Gauss_cell, parameters_threshold_method, parameters_threshold_percentile);
%     th_scaffold = compute_threshold(bg_Gauss_scaffold, parameters_threshold_method, parameters_threshold_percentile);
%               
%     % Binarization
%     BW_cell = (Z_Gauss_cell > th_cell);
%     BW_scaffold = (Z_Gauss_scaffold > th_scaffold);
%     if bMorph
%         for kl = 1:L
%             bw_cell = BW_cell(kl,:,:);
