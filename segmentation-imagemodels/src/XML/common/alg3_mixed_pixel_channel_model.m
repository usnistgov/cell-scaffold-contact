...
function [status, message] = alg3_mixed_pixel_channel_model(xmlDocument)
%% Algorithm 3: Mixed-pixel channel model
% Inputs
...
for kd = 1:nD
    ...
    % Binarize z-stacks
    BW_raw_cell = (Z_cell > th_cell);
    BW_raw_scaffold = (Z_scaffold > th_scaffold);
    if bMorph
        for kl = 1:L
            bw_cell = squeeze(BW_raw_cell(kl,:,:));
            bw_scaffold = squeeze(BW_raw_scaffold(kl,:,:));
            
            bw_cell = imopen(bw_cell, SE);
            bw_scaffold = imopen(bw_scaffold, SE);
            
            BW_raw_cell(kl,:,:) = bw_cell;
            BW_raw_scaffold(kl,:,:) = bw_scaffold;
        end
    end
    
    % Correlation coefficient
    r_cell = compute_corr_coeff(Z_cell, Z_scaffold, BW_raw_cell);
    r_scaffold = compute_corr_coeff(Z_cell, Z_scaffold, BW_raw_scaffold);
        
    clear BW_raw_cell BW_raw_scaffold
    
    % Compute flat-field correction and probability for each channel
    max_Z_cell = max(Z_cell(:));
    max_Z_scaffold = max(Z_scaffold(:));
    FFC_cell = compute_FFC_corr_coeff(Z_cell, bg_img_cell, r_cell, max_Z_cell, max_Z_scaffold);
    FFC_scaffold = compute_FFC_corr_coeff(Z_scaffold, bg_img_scaffold, r_scaffold, max_Z_cell, max_Z_scaffold);
