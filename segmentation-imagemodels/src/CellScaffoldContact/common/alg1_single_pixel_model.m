...
function [status, message] = alg1_single_pixel_model(xmlDocument)
%% Algorithm 1: Single pixel model
% Inputs
...
for kd = 1:nD
    ...
    BW_cell = (Z_cell > th_cell);
    BW_scaffold = (Z_scaffold > th_scaffold);
    if bMorph
        for kl = 1:L
            bw_cell = squeeze(BW_cell(kl,:,:));
            bw_scaffold = squeeze(BW_scaffold(kl,:,:));
    ...
    % Max. intensity values in foreground
    max_fg_cell = max(Z_cell(BW_cell));
    max_fg_scaffold = max(Z_scaffold(BW_scaffold));
        
    % Probability
    if max_fg_cell > 0
        P_cell = Z_cell / max_fg_cell;
    else
        P_cell = zeros(L,H,W);
    end

    if max_fg_scaffold > 0
        P_scaffold = Z_scaffold / max_fg_scaffold;
    else
        P_scaffold = zeros(L,H,W);
    end
...
%     clear Label_cell Label_scaffold Label_contact

    clear BW_cell BW_scaffold
