...
function Z2 = dilate_3D(Z, nNeighborhood)

[L,H,W] = size(Z);

Z2 = zeros(L,H,W);
for kl = 1:L
    Z2(kl,:,:) = ordfilt2(squeeze(Z(kl,:,:)), nNeighborhood^2, ones(nNeighborhood));
end

for kh = 1:H
    Z2(:,kh,:) = ordfilt2(squeeze(Z2(:,kh,:)), nNeighborhood, ones(nNeighborhood,1));
end