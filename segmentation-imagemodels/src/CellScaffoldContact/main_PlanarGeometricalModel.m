...
%% Cell-scaffold contact point estimation: Planar geometrical model for spun coat

% xml_doc_path = './xml/test_fitPlane.xml';
% [status, message] = FitPlane(xml_doc_path);
% if status > 0
%     disp(message);
% end

xml_doc_path = './xml/test_fitPlane_CellIntersection.xml';
[status, message] = FitPlane_CellIntersection(xml_doc_path);
if status > 0
    disp(message);
end
