...
public class Nifti_Reader extends ImagePlus {
	...
	public ImagePlus read(String inputFilename) {
		File file = new File(inputFilename);
		
		//OpenDialog od = new OpenDialog("Open Nifti...", arg);
		String directory = file.getParent();//od.getDirectory();
		String name = file.getName();//od.getFileName();
		...
		if (imp!=null) {
			...
			//if (cal_max != cal_min) getProcessor().setMinAndMax(cal_min, (getType()==GRAY32) ? cal_max : cal_max-1.0);
	...
	public ImagePlus load(String directory, String name) {
		...
		System.out.println("Reading Header File: " + directory + hdrName);
		try {
			fi = readHeader( directory+hdrName );
			if (fi==null) return null; 	
		} catch (IOException e) { 
			//IJ.log("FileLoader: "+ e.getMessage()); 
			System.out.println("FileLoader: "+ e.getMessage());
		}
		if (isNiftiData) { 
			//IJ.showStatus("Reading Nifti File: " + directory + imgName );
			System.out.println("Reading Nifti File: " + directory + imgName ); 
		} else { 
			//IJ.showStatus("Reading Analyze File: " + directory + imgName ); 
			System.out.println("Reading Analyze File: " + directory + imgName );
		}
		fi.fileName = imgName;
...
// data_history 
	...
	public boolean checkDataRange(ImagePlus imp) { 
		int min = 65536, max = 0;
	...
	public static void main(String[] args) {
		...
		String inputFilename = new String("C:/PeterB/Projects/CarlSimon/ContactProb-test/Results/061115_SC_1_63x_Pos049.fits");
		ImagePlus img3D = new ImagePlus(inputFilename); // input one fits file per zstack
		Nifti_Writer test = new Nifti_Writer();
		String outputType = new String("::NIFTI_FILE:");
		String outFilename = new String("C:/PeterB/Projects/CarlSimon/ContactProb-test/Results/test/061115_SC_1_63x_Pos049.nii");
		test.write(img3D, outFilename, outputType);
		
		Nifti_Reader testRead = new Nifti_Reader();
		ImagePlus img2_3D = testRead.read(outFilename);
		String outFilename2 = new String("C:/PeterB/Projects/CarlSimon/ContactProb-test/Results/test/061115_SC_1_63x_Pos049_v2.nii");
		test.write(img2_3D, outFilename2, outputType);
