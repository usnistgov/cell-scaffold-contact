...
public class Nifti_Writer {
		...
		public void write(ImagePlus imp, String outFilename, String outputType) {
			...
			if (outputType !=null) { 
				if (outputType.startsWith("::ANALYZE_7_5:")) {
					output_type = ANALYZE_7_5; 
					outputType=outputType.substring(14).trim();
				} else if (outputType.startsWith("::NIFTI_ANALYZE:")) { 
					output_type = NIFTI_ANALYZE; 
					outputType=outputType.substring(16).trim();
				} else if (outputType.startsWith("::NIFTI_FILE:")) { 
					output_type = NIFTI_FILE; 
					outputType=outputType.substring(13).trim(); 
				}
			}
			String fType = "", suffix = "";
				switch(output_type) { 
				...
				directory = filename.getParent();//sd.getDirectory();
				name = filename.getName();//sd.getFileName();

			
			if (name == null || name == "") {
				//IJ.showStatus(""); 
				//IJ.error("No filename selected");
				System.err.println("No filename selected");				
				return;
			}
...
/*				Calibration c = imp.getCalibration();
		...
		public boolean save(ImagePlus imp, String directory, String name) {
			...
			try {
				...
				System.out.println("Saving as Analyze: " + directory + name);
				...
				System.out.println("Nifti_Writer: "+ e.getMessage());
				return false;	
			}
		} 
		...
		private void writeRGBPlanar(ImagePlus imp, OutputStream out) throws IOException { 
			...
			for (int i=1; i<=nImages; i++) { 
				...
				System.out.println((double) i/nImages );
			}
		}
...
/*	                short[] pixels = (short[])ip.getPixels();
	                ...
	                short[] pixels = (short[])ip.getPixels();
	                for (int i=0; i<pixels.length; i++){
	                	int value32bit = (pixels[i]&0xffff);
	                	pixels[i] = (short)(value32bit+value);
	                }
	            }
	         }
